...
async def main() -> None:
    ...
    try:
        # Wait for the first thing to finish (either OK or with exception)
        done, pending = await asyncio.wait(
            all_tasks, return_when=asyncio.FIRST_COMPLETED
        )

        # If any task finished with an exception, raise it now
        for t in done:
            if t.exception() is not None:
                raise t.exception()

        # If we get here, one task finished normally.
        # We still want to propagate exceptions from the rest, so:
        await asyncio.gather(*all_tasks)  # Will raise immediately on any error

    finally:
        # Always cancel everything to avoid hanging
        for t in all_tasks:
            if not t.done():
                t.cancel()
        # Give them a moment to shut down (optional but polite)
