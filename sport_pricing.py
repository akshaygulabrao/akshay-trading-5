...
def contract_price_odds(home_odds, away_odds):
    ...
    return adjusted_home, adjusted_away  # Always returns (home, away)

def contract_price_home(home_odds,away_odds):
    home_odds, away_odds = contract_price_odds(home_odds,away_odds)
    if home_odds < .15 or home_odds > .85:
        bid_home,ask_home = (home_odds -.02, home_odds + .02)
        bid_away,ask_away = (away_odds -.02, away_odds + .02)
    else:
        bid_home,ask_home = (home_odds -.03, home_odds + .03)
        bid_away,ask_away = (away_odds -.03, away_odds + .03)
    return math.floor(bid_home*100),math.ceil(ask_home*100)

def contract_price_away(home_odds,away_odds):
    home_odds, away_odds = contract_price_odds(home_odds,away_odds)
    if home_odds < .15 or home_odds > .85:
        bid_home,ask_home = (home_odds -.02, home_odds + .02)
        bid_away,ask_away = (away_odds -.02, away_odds + .02)
        ...
        bid_home,ask_home = (home_odds -.03, home_odds + .03)
        bid_away,ask_away = (away_odds -.03, away_odds + .03)
    return math.floor(bid_away*100),math.ceil(ask_away * 100)


bid,ask = contract_price_away(230,-285)
