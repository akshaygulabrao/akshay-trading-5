...
def format_timedelta_compact(td: timedelta) -> str:
    ...
    if seconds or not parts:
        parts.append(f"{seconds}s")  # Always show at least seconds
    return " ".join(parts)
...
def get_exchange_status(current_time: datetime) -> tuple:
    ...
    return (
        "open" if current_time < exchange_close else "closed",
        exchange_close,
        time_remaining,
    )
...
async def ui():
    ...
    while True:
        ...
        for kalshi_site in kalshi_sites:
            ...
            print(
                f"{kalshi_site.ljust(5)} S:{lsr_str}, SE: {lsr_err:.02f}",
                end=" forecasts:",
            )
            ...
            for day in site2days[kalshi_site]:
                ...
                for mkt in kalshi_site_day2mkts.get((kalshi_site, day), []):
                    print(
                        f"    {mkt.ljust(25)} {site_day_mkt2price[(kalshi_site,day,mkt)]:.02f}"
                    )
...
async def get_lsr():
    ...
    while True:
        for site in kalshi_sites:
            ...
            kalshi_site2sensor_forecast_diff[site] = (
                latest_sensor_temp - k_site2efp[site]
            )
